...
public class AboutDialog extends JDialog
{
    ...
    private void loadLicense()
    {
        if (m_licenseTextPane.getText().length() > 0)
            return;
        // read the license as text file from resource
        try (InputStream inputStream = getClass().getResourceAsStream("/resource/text/LICENSE.txt");
             BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream))) {
            StringBuilder content = new StringBuilder();
            String line;
            // loop to iterate until reading the last line
            while ((line = reader.readLine()) != null) {
                // append the content while reading the file
                content.append(line).append("\n");
            }

            m_licenseTextPane.setText(content.toString());
